...
1. Create a method called 'registerNewAnswer' on the 'poll' object. 
  ...
  1.2. Based on the input number, update the answers array. 
	   For example, if the option is 3, increase the value 
	   AT POSITION 3 of the array by 1. Make sure to check if 
...
BONUS: Use the 'displayResults' method to display the 2 arrays 
	   in the test data. Use both the 'array' and the 'string' 
	   option. Do NOT put the arrays in the poll object! 
...
const poll = {
	...
	registerNewAnswer() {
		let str = "";
		for (const [number, language] of this.languages) {
			str = str + `${number}: ${language} \n`;
		}
		const question = Number(prompt("What is your favourite programming language?\n" + str + "(Write option number)"));
		if (typeof question === "number" && (question >= 0 && question < 4)) {
			for (const [key, value] of this.languages) {
				if (question === key) this.answers[key]++;
			}
		} else {
			console.log("No language selected. Choose between 0 and 3");
		}
		this.displayResults();
