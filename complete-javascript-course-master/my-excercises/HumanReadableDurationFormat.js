...
For the purpose of this Kata, a year is 365 days and a day is 24 
hours.
...
units of time, separated by a space. The unit of time is used 
...
Hence, 1 minute and 0 seconds is not valid, but it should be 
just 1 minute.

A unit of time must be used "as much as possible". It means that 
the function should not return 61 seconds, but 1 minute and 1 
second instead. Formally, the duration specified by of a component 
...
const formatDuration = function(seconds) {
  ...
  for (const [key, value] of Object.entries(time)) {
    if (seconds >= value) {
      let val = Math.floor(seconds/value);
