...
cities for months from January to December. The records of towns 
...
for the city town and the strng data or data1 (In R and Julia this 
function is called avg).
...
see "Sample Tests:") are adapted from: 
...
const mean = function(town, str) {
	const record = String(str.split(/\n/).filter(rec => rec.slice(0, rec.indexOf(":")) === town));
	if(towns.includes(town) && record) {
		return record.match(/[+-]?\d+(\.\d+)?/g).map(num => parseFloat(num)).reduce((acc, mov) => acc + mov / 12, 0);
	}
	return -1;
}

const variance = function(town, str) {
	const record = String(str.split(/\n/).filter(rec => rec.slice(0, rec.indexOf(":")) === town));
	if(towns.includes(town) && record) {
		return record.match(/[+-]?\d+(\.\d+)?/g).map(num => parseFloat(num)).reduce((acc, mov) => acc + (mov - mean(town, str))**2 / 12, 0);
	}
	return -1;
};
...
console.log(variance("Montevideo", data));
